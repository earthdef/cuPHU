...
/* cuPHU patch: all file-scope solver state made static _Thread_local so
 * concurrent SolveCS2 calls from different threads (one per tile) do not
 * share state.  Heap allocations are freed inside SolveCS2 before return,
 * so per-thread state carries nothing across calls. */
...
static _Thread_local long n_push      =0,
     n_relabel   =0,
     n_discharge =0,
     n_refine    =0,
     n_update    =0,
...
static _Thread_local long   n,       /* number of nodes */
       m;                    /* number of arcs */

static _Thread_local short   *cap;    /* array containig capacities */

static _Thread_local node   *nodes,  /* array of nodes */
       *sentinel_node,       /* next after last */
       *excq_first,          /* first node in push-queue */
       *excq_last;           /* last node in push-queue */

static _Thread_local arc    *arcs,   /* array of arcs */
       *sentinel_arc;        /* next after last */

static _Thread_local bucket *buckets, /* array of buckets */
       *l_bucket;            /* last bucket */
static _Thread_local long   linf;    /* number of l_bucket + 1 */
static _Thread_local double dlinf;   /* copy of linf in double mode */

static _Thread_local int time_for_price_in;
static _Thread_local double epsilon, /* optimality bound */
...
static _Thread_local double total_excess;  /* total excess */

static _Thread_local long   n_rel,   /* number of relabels from last price update */
       n_ref,                /* current number of refines */
       n_src;                /* current number of nodes with excess */

static _Thread_local int   flag_price = 0, /* if = 1 - signal to start price-in ASAP -
...
static _Thread_local long  empty_push_bound; /* maximal possible number of zero pushes
                                during one discharge */

static _Thread_local int   snc_max;  /* maximal number of cycles cancelled
                                during price refine */

static _Thread_local arc   d_arc;    /* dummy arc - for technical reasons */

static _Thread_local node  d_node,   /* dummy node - for technical reasons */
